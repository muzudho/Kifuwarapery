...
EvalSum Evaluation09::doapc(const Position& pos, const int index[2]) {
	...
	const auto* pkppw = Evaluater::KPP[Util_Square::Inverse(sq_wk)][index[1]];
...
bool Evaluation09::calcDifference(Position& pos, SearchStack* ss) {
	...
	if (lastMove.pieceTypeFrom() == King) {
		...
		if (pos.turn() == Black) {
			const auto* ppkppw = Evaluater::KPP[Util_Square::Inverse(sq_wk)];
			...
			for (int i = 0; i < pos.nlist(); ++i) {
				...
				diff.p[2][0] -= Evaluater::KKP[Util_Square::Inverse(sq_wk)][Util_Square::Inverse(sq_bk)][k1][0];
				diff.p[2][1] += Evaluater::KKP[Util_Square::Inverse(sq_wk)][Util_Square::Inverse(sq_bk)][k1][1];
			}
		...
		if (pos.cl().size == 1) {
			...
			diff.p[1] -= Evaluater::KPP[Util_Square::Inverse(pos.kingSquare(White))][pos.cl().clistpair[0].newlist[1]][pos.cl().clistpair[1].newlist[1]];
			const int listIndex_cap = pos.cl().listindex[1];
			pos.plist0()[listIndex_cap] = pos.cl().clistpair[1].oldlist[0];
			pos.plist1()[listIndex_cap] = pos.cl().clistpair[1].oldlist[1];
			...
			diff.p[1] += Evaluater::KPP[Util_Square::Inverse(pos.kingSquare(White))][pos.cl().clistpair[0].oldlist[1]][pos.cl().clistpair[1].oldlist[1]];
			pos.plist0()[listIndex_cap] = pos.cl().clistpair[1].newlist[0];
			pos.plist1()[listIndex_cap] = pos.cl().clistpair[1].newlist[1];
		}
...
int Evaluation09::make_list_unUseDiff(const Position& pos, int list0[EvalList::ListSize], int list1[EvalList::ListSize], int nlist) {
	auto func = [&](const Bitboard& posBB, const int f_pt, const int e_pt) {
		...
		FOREACH_BB(bb, sq, {
			list0[nlist] = (f_pt)+sq;
		list1[nlist] = (e_pt)+Util_Square::Inverse(sq);
		nlist += 1;
		});
		bb = (posBB)& pos.bbOf(White);
		FOREACH_BB(bb, sq, {
			list0[nlist] = (e_pt)+sq;
		list1[nlist] = (f_pt)+Util_Square::Inverse(sq);
		nlist += 1;
		});
	};
