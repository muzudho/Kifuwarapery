...
Move UsiOperation::UsiToMoveBody(const Position& pos, const std::string& moveStr) {
	...
	if (g_charToPieceUSI.IsLegalChar(moveStr[0])) {
		...
		const Square to = UtilSquare::FromFileRank(toFile, toRank);
		if (moveStr[4] == '\0') {
			move = g_makePromoteMove.GetSelectedMakeMove_ExceptPromote(N00_Capture, UtilPiece::ToPieceType(pos.GetPiece(from)), from, to, pos);
		}
		else if (moveStr[4] == '+') {
			if (moveStr[5] != '\0') {
				return g_MOVE_NONE;
...
Move UsiOperation::CsaToMoveBody(const Position& pos, const std::string& moveStr) {
	...
	if (moveStr[0] == '0' && moveStr[1] == '0') {
		...
		if (ptFrom == ptTo) {
			// non promote
			move = g_makePromoteMove.GetSelectedMakeMove_ExceptPromote(N00_Capture, ptFrom, from, to, pos);
		}
