...
public:
	...
	static void MakeDropMovesToRank9ExceptNL(
		...
		Square to;
		Bitboard toBB = target & TRank9BB; FOREACH_BB(toBB, to, { Unroller<3>()([&](const int i) { (*moveStackList++).m_move = UtilMove::MakeDropMove(haveHand[noKnightLanceIdx + i], to); }); });
	...
	static void MakeDropMovesToRank8ExceptN(
		...
		Square to;
		Bitboard toBB = target & TRank8BB; FOREACH_BB(toBB, to, { Unroller<3>()([&](const int i) { (*moveStackList++).m_move = UtilMove::MakeDropMove(haveHand[noKnightIdx + i], to); }); });
	...
	static void MakeDropMovesToRank1234567(
		...
		Square to;
		FOREACH_BB(toBB, to, { Unroller<3>()([&](const int i) { (*moveStackList++).m_move = UtilMove::MakeDropMove(haveHand[i], to); }); });
		//Square to;
		//FOREACH_BB( toBB, to, {
