...
public:
	...
	MoveStack* MakeDropMovesToRank8ExceptN(
		...
		while (toBB.GetP(0)) {
			...
			pMovestack->m_move = ConvMove::Convert30_MakeDropMove_da(ConvMove::FROM_PIECETYPE_DA10(haveHandArr[dmEvent.m_noKnightIdx + 4]), iTo);
			pMovestack++;
	...
	MoveStack* MakeDropMovesToRank1234567(
		MoveStack* pMovestack,
		const DropMakerEvent& dmEvent,
		PieceType haveHandArr[6]// TODO: これ、打ムーブにできないかだぜ☆？（＾ｑ＾）
	) const override {
